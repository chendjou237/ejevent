...
export function Cart({ translations }: CartProps) {
  ...
  const handleSubmit = async (e: { preventDefault: () => void }) => {
    e.preventDefault()
    toast.loading(translations.toasts.creatingBooking)
    ...
    if (response.status === 'error') {
      toast.error(translations.toasts.bookingError)
    }
    if (response.status === 'success') {
      setIsSuccess(true)
      toast.success(translations.toasts.bookingSuccess)
      setCart([])
      ...
      <div className="grid grid-cols-1 md:grid-cols-[2fr_1fr] gap-8">
              ...
              <div className="flex-1">
                ...
                <div className="mt-2 text-sm text-muted-foreground ">
                  <span className='font-medium'>
                    {translations.orderSummary.startDate}: {format(item.start_at, "MM-dd")}
                  </span>
                  <Separator orientation="vertical" className="mx-2" />
                  <span className='font-medium'>
                    {translations.orderSummary.endDate}: {format(item.end_at, "MM-dd")}
                  </span>
                </div>
              </div>
        ...
        <div className="space-y-6 ">
          <Card className='opacity-90'>
            <CardHeader>
              <CardTitle>{translations.orderSummary.title}</CardTitle>
            </CardHeader>
            <CardContent className="space-y-4">
              <div className="flex items-center justify-between">
                <span>{translations.orderSummary.decorations}</span>
                <span>{products}</span>
              </div>
              <div className="flex items-center justify-between">
                <span>{translations.orderSummary.services}</span>
                <span>{services}</span>
              </div>
              <Separator />
            ...
            <CardFooter>
              <Button onClick={handleAddR} className="w-full">{translations.orderSummary.addReservations}</Button>
            </CardFooter>
          </Card>
          <Card className='opacity-90'>
            <CardHeader>
              <CardTitle>{translations.personalInfo.title}</CardTitle>
            </CardHeader>
            <CardContent>
              <form onSubmit={handleSubmit} className="space-y-4">
                <div className="space-y-1">
                  <Label htmlFor="name">{translations.personalInfo.name.label}</Label>
                  <Input
                    id="name"
                    placeholder={translations.personalInfo.name.placeholder}
                ...
                <div className="space-y-1">
                  <Label htmlFor="email">{translations.personalInfo.email.label}</Label>
                  <Input
                    id="email"
                    type="email"
                    placeholder={translations.personalInfo.email.placeholder}
                ...
                <div className="space-y-1">
                  <Label htmlFor="phone">{translations.personalInfo.phone.label}</Label>
                  <Input
                    id="phone"
                    type="tel"
                    placeholder={translations.personalInfo.phone.placeholder}
                ...
                <Button type="submit" disabled={isCreatingBooking} className="w-full">
                  {translations.personalInfo.confirmButton}
                </Button>
              </form>
            </CardContent>
          </Card>
